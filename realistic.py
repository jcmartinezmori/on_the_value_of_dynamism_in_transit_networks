...
def main():
    ...
    print('     loading data')
    with open('./realistic/g.pkl', 'rb') as file:
        g = pickle.load(file)
    g_dists = pd.read_csv('./realistic/g_dists.csv', index_col=0).rename(columns=lambda x: int(x))
    df = pd.read_csv('./realistic/df.csv', parse_dates=['s_dt', 't_dt'])
    ...
    for job in results_df_jobs:
        ...
        results.append([job_delta_t, job_rho, job_g, job_ct, job_mst_cost, job_opt, job_theta])
        ...
        for bin_no in range(no_bins):
            ...
            if len(pairs) > 0:
                ...
                results_df = pd.DataFrame(results, columns=['delta_t', 'rho', 'g', 'ct', 'mst_cost', 'opt', 'theta'])
...
def curve_plotter():
    ...
    results_df = pd.read_pickle('./realistic/results_df.pkl')
    results_df.sort_values('delta_t', inplace=True)
    results_df['delta_t'] /= 60
    results_df['numerator'] = results_df['theta'] * results_df['delta_t']
    ...
    fig.write_image('./images/realistic_curves.png', width=w, height=h, scale=1)
...
def map_plotter():
    ...
    for _, row in results_df.iterrows():
        ...
        _map.save('./images/delta_t_{0}_rho_{1}_g.html'.format(row.delta_t, round(row.rho, 2)))
